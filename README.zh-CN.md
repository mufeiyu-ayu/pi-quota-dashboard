...
- **一行，不是三行。** pi 内置 footer 是三行（工作目录 / 统计 / 扩展状态），这里把后两行合并。
...
pi 内置 footer 是三行。本扩展用 `ui.setFooter()` 把统计行和扩展状态行合成一行，用 `│` 分段：
...
| 终端宽度 | 显示 |
|---|---|
| 宽 | `↑31k ↓518 R91k W2.0k CH73.4% │ █░░░░░░░░░ 10.4% 104k/1.0M │ $0.426 │ 状态` |
| 中 | 丢 token 明细 |
| 窄 | 再丢绝对计数 —— 进度条和百分比已经说明同一件事 |
| 更窄 | 只截右侧模型名。额度和其他扩展状态任何一级都不丢 |
...
其余（token 计数、缓存命中率、上下文着色阈值、模型右对齐、provider 前缀、截断规则）与内置 footer 一致。
